 ...
 * @link      http://pear.typo3.org
 */
class MetaModels_Sniffs_WhiteSpace_AssignmentArithmeticAndComparisonSpaceSniff implements PHP_CodeSniffer_Sniff {
    /**
        ...
        if ($tokens[$stackPtr]['code'] === T_MINUS) {
            ...
            if ($tokens[$number]['code'] === T_LNUMBER || $tokens[$number]['code'] === T_DNUMBER) {
                ...
                if ($previous !== FALSE) {
                    ...
                    $isTenary           = (($tokens[$previous]['code'] === T_INLINE_THEN) || ($tokens[$previous]['code'] === T_COLON));

                    if (($isAssignment === TRUE) || ($isEquality === TRUE) || ($isComparison === TRUE) || ($isArithmetic === TRUE) || ($isFunctionOrArray === TRUE) || ($isFunctionArgument === TRUE) || ($isTenary === TRUE)) {
    ...
    /**
     * Calculates the expected string to give a hint in error message
     *
     * @param  string $mode
     * @param  array  $tokens
     * @param  int    $stackPtr
    ...
    /**
     * Detects if the given token a whitespace
     *
     * @param  string  $mode
     * @param  array   $tokens
     * @param  int     $stackPtr
